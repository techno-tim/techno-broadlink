...
def discover_devices(host_ip):
    ...
    parser.add_argument("--ip", default=host_ip,
                        help="ip address to use in the discovery")
    parser.add_argument("--dst-ip", default=broadcast_address,
                        help="destination ip address to use in the discovery")
    args = parser.parse_args()
    print(f'args: {args}')
    print("Discovering...")
    devices = broadlink.discover(
        timeout=args.timeout, local_ip_address=args.ip, discover_ip_address=args.dst_ip)
        ...
        try:
            ...
            if auth_state:
                if device.host[0] == ip_address:
                    ...
                    learned = format_durations(
                        to_microseconds(bytearray(data)))
                    learned = ''.join(format(x, '02x')
            ...
            if auth_state:
                if device.host[0] == ip_address:
                    ...
                    if os.path.exists(file_with_path):
                        ...
                        with open(file_with_path) as existing_file:
                            data = json.load(existing_file)
                            existing_command = list(
                                filter(lambda x: x['id'] == command_id, data['commands']))
                            if data and data['mac'] and existing_command and existing_command[0] and existing_command[0]['id']:
                                device.send_data(bytearray.fromhex(
                                    ''.join(existing_command[0]['data'])))
                                print("###########################################")
                                print("command sent!")
                                command_status = {
                                    "success": True,
            ...
            if auth_state:
                if device.host[0] == ip_address:
                    ...
                    if os.path.exists(file_with_path):
                        ...
                        print("Error authenticating with device : {}".format(
                            device.host))
